...
#PBS -N Braker_annotation
#PBS -l nodes=1:ppn=8,vmem=40gb,walltime=1:00:00

##installation of Braker 2.1.2 on carbonate following https://github.com/Gaius-Augustus/BRAKER, however we need to install bamtools locally for augustus make following ttps://iamphioxus.org/2017/05/08/installing-augustus-with-manual-bamtools-installation/ 
## and load bcftools samtools htslib-1.9 from module and then change the path in (..)/Augustus/auxprogs/bam2wig/Makefile SAMTOOLS=/N/soft/rhel7/samtools/1.9; HTSLIB=/N/soft/rhel7/samtools/1.9/htslib-1.9; BCFTOOLS=/N/soft/rhel7/bcftools/1.5 
...
cd /N/dc2/projects/Tswallow_GLRP/CT/Braker_annotation/test_run
braker.pl --species=Tachycineta_bicolor --genome=Renamed_tswallow_contig.fasta \
       --bam=Renamed_tswallow_contig.bam --prot_seq=ave_proteome.fasta \
